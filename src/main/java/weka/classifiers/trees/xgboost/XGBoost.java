...
//            predictDouble[i] = Double.valueOf(String.valueOf(predict[i]));
    ...
    private double[] predictWithPredictor(Instance instance) {
        ...
        float[] predictionsFloat = predictor.predict(fVecSparse);

	     // Convert float[] to double[]
	     double[] predictions = new double[predictionsFloat.length];
	     for (int i = 0; i < predictionsFloat.length; i++) {
	         predictions[i] = predictionsFloat[i];
	     }
        return predictions;
    }
